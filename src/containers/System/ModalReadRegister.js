...
class Modalread_register extends Component {
    constructor(props) {
        super(props)
        this.state = {
            read_register: [
    ...
    componentDidMount() {
        let {read_registerEdit} = this.props
        if (read_registerEdit && !_.isEmpty(read_registerEdit)) {
            this.setState({
                read_register: read_registerEdit,
                indexElement: read_registerEdit.length - 1,
            })
        } else {
            this.setState({
                read_register: [
    ...
    handleOnChangeInput = (event, name, index) => {
        let copyItem = this.state.read_register
        copyItem[index] = this.state.read_register[index]
        copyItem[index][name] = event.target.value
        this.setState({
            read_register: copyItem,
        })
    }

    handleOnClickAdd = () => {
        let copyItem = this.state
        copyItem.read_register.push({
            tagName: '',
            dataType: '',
            unit: '',
            offset: '',
        ...
        this.setState({
            read_register: copyItem.read_register,
    ...
    handleOnClickMinus = (index) => {
        let copyItem = this.state
        copyItem.read_register.splice(index, 1)
        copyItem.indexElement = this.state.indexElement - 1
        this.setState({
            read_register: copyItem.read_register,
            indexElement: copyItem.indexElement,
    ...
    checkValidInput = () => {
        ...
        for (let i = 0; i <= indexElement; i++) {
            for (let j = 0; j < arrInput.length; j++) {
                if (!this.state.read_register[i][arrInput[j]]) {
                    isValid = false
    ...
    handleEditDeviceTypeOne = () => {
        let isValid = this.checkValidInput()
        if (isValid) {
            this.props.handleEditDeviceTypeOne('openReadRegister', this.state)
        }
    }

    render() {
        let numberLoop = this.state.read_register
        return (
            <Modal
...
export default connect(mapStateToProps, mapDispatchToProps)(Modalread_register)
