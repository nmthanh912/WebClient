...
class ModalReadRegister extends Component {
    ...
    componentDidMount() {
        let {watch_registerEdit} = this.props
        if (watch_registerEdit.watch_register && !_.isEmpty(watch_registerEdit.watch_register)) {
            this.setState({
                watch_register: watch_registerEdit.watch_register,
            })
        } else {
            this.setState({
                watch_register: [
    ...
    handleOnChangeInput = (event, id, index) => {
        let copyItem = this.state.watch_register
        copyItem[index] = this.state.watch_register[index]
        copyItem[index][id] = event.target.value
        this.setState({
            watch_register: copyItem,
        })
    }

    handleOnClickAdd = () => {
        let copyItem = this.state
        copyItem.watch_register.push({
            tagName: '',
            dataType: '',
            unit: '',
            offset: '',
        ...
        this.setState({
            watch_register: copyItem.watch_register,
    ...
    handleOnClickMinus = (index) => {
        let copyItem = this.state
        copyItem.watch_register.splice(index, 1)
        copyItem.indexElement = this.state.indexElement - 1
        this.setState({
            watch_register: copyItem.watch_register,
            indexElement: copyItem.indexElement,
    ...
    checkValidInput = () => {
        ...
        for (let i = 0; i <= indexElement; i++) {
            for (let j = 0; j < arrInput.length; j++) {
                if (!this.state.watch_register[i][arrInput[j]]) {
                    isValid = false
    ...
    handleEditDeviceTypeOne = () => {
        let isValid = this.checkValidInput()
        if (isValid) {
            this.props.handleEditDeviceTypeOne('openWatchRegister', this.state)
        }
    }

    render() {
        let numberLoop = this.state.watch_register
        return (
            <Modal
